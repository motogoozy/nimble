module.exports = {
  ...
  createTask: async (req, res, next) => {
    ...
    const db = req.app.get('db');
    try {
      let added = await db.task.create_task({
        project_id,
        title,
        status,
        created_by,
  ...
  updateTask: async (req, res, next) => {
    ...
    const db = req.app.get('db');
    try {
      let updatedTask = await db.task.update_task({
        task_id,
        title,
        notes,
        status,
  ...
  addTaskUser: async (req, res, next) => {
    ...
    try {
      let added = await db.task_users.add_task_user({
        task_id,
        user_id,
        project_id,
      });
  ...
  deleteTaskUsersByTask: async (req, res, next) => {
    ...
    try {
      let deleted = await db.task_users.delete_all_task_users_by_task({
        task_id,
      });
      res.status(200).send(deleted[0]);
    } catch (err) {
