module.exports = {
  ...
  getProjectPermissions: async (req, res, next) => {
    ...
    const db = req.app.get('db');
    try {
      let permissions = await db.project.get_project_permissions({
        project_id,
      });
  ...
  createProject: async (req, res, next) => {
    ...
    try {
      ...
      await db.project.add_project_permissions({
        project_id: added[0].project_id,
      });
  ...
  addProjectUser: async (req, res, next) => {
    ...
    const db = req.app.get('db');
    try {
      let addedUser = await db.project.add_project_user({
        project_id,
        user_id,
      });
  ...
  updateProject: async (req, res, next) => {
    ...
    const db = req.app.get('db');
    try {
      let updatedProject = await db.project.update_project({
        project_id,
        title,
        list_order,
      });
  ...
  updateProjectPermissions: async (req, res, next) => {
    ...
    const db = req.app.get('db');
    try {
      let updatedPermissions = await db.project.update_project_permissions({
        ...permissions,
        project_id,
      });
  ...
  deleteProjectUser: async (req, res, next) => {
    ...
    const db = req.app.get('db');
    try {
      let deletedUser = await db.project.delete_project_user({
        project_id,
        user_id,
      });
